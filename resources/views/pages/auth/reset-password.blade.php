...
<x-layouts::auth title="Reset Password" description="This is the page where you can reset your password.">
    <form method="post" action="{{ route('password.update') }}">
        ...
        <x-input-group class="mt-4">
            ...
            <x-text-input type="password" name="password" id="form-password" required autofocus autocomplete="new-password" passwordrules="{{ Password::default()->toPasswordRulesString() }}" />
            @error('password')
                <x-error>{{ $message }}</x-error>
            @enderror
        </x-input-group>
