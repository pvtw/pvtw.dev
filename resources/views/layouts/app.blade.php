        ...
        <div id="page" class="flex flex-col min-h-dvh">
            ...
            <header class="bg-primary text-white flex flex-col sm:flex-row items-start sm:items-center justify-between {{ $header_position }} top-0 w-full shadow z-[9001]"
                ...
                <nav id="primary-navigation" class="w-full sm:w-auto h-[calc(100vh-4rem)] sm:h-auto overflow-y-auto sm:overflow-y-visible sm:mr-2 p-2 sm:p-0" x-cloak x-show="open">
                    <ul class="flex flex-col sm:flex-row sm:items-center gap-2">
                        <li>
                            <a
                        ...
                        @auth
                            <li>
                                <details id="myAccountMenu" class="sm:relative border-t border-fuchsia-500 sm:border-none" @click.away="document.getElementById('myAccountMenu').removeAttribute('open')">
                                    <summary class="cursor-pointer block w-full sm:w-12 h-12 flex justify-between sm:justify-center items-center pr-2 sm:p-0 outline-none focus-visible:ring focus-visible:ring-blue-500">
                                        <span class="sm:sr-only block text-xl text-nowrap font-bold px-4 pt-2 pb-1 border-b-4 border-transparent hover:border-fuchsia-500 transition-colors duration-200">My Account</span>
                                        @if(null !== auth()->user()->avatar_url)
                                            <img src="{{ auth()->user()->avatar_url }}" alt="{{ auth()->user()->name }}'s avatar" class="w-8 h-8 rounded-full">
                                        @else
                                    ...
                                    <ul class="sm:absolute sm:top-12 sm:right-0 bg-gray-100 dark:bg-gray-900 text-black dark:text-white p-2 rounded shadow">
                                        <li>
                                            <form method="post" action="{{ route('logout') }}">
                                                @csrf

                                                <button type="submit" class="block w-full sm:w-auto text-xl text-nowrap text-left font-bold px-4 pt-2 pb-1 border-b-4 border-transparent hover:border-fuchsia-500 transition-colors duration-200 outline-none focus-visible:ring focus-visible:ring-blue-500">Logout</button>
                                            </form>
                                        </li>
                                    </ul>
                                </details>
